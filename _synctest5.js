...
(async()=>{
  ...
  await page.evaluate(()=>{
    window.setupCompareSyncInContainer = function(container){
      ...
      const initialOrbit=new Map(),initialTarget=new Map(),initialFov=new Map(),initialMaxFov=new Map();
      const MIN_ZOOM_RADIUS_RATIO=0.2,MIN_ZOOM_FOV_RATIO=0.4;
      function recordInitial(mv){if(initialOrbit.has(mv)||!mv.loaded)return;try{const o=mv.getCameraOrbit(),tg=mv.getCameraTarget(),fov=mv.getFieldOfView();if(o&&o.radius>0){initialOrbit.set(mv,{theta:o.theta,phi:o.phi,radius:o.radius});initialTarget.set(mv,{x:tg.x,y:tg.y,z:tg.z});initialFov.set(mv,fov);initialMaxFov.set(mv,mv.getMaximumFieldOfView());mv.minCameraOrbit='auto auto '+(o.radius*MIN_ZOOM_RADIUS_RATIO).toFixed(4)+'m';mv.minFieldOfView=''+(fov*MIN_ZOOM_FOV_RATIO).toFixed(2)+'deg';window.__init=window.__init||{};window.__init[mv.id]={o:{theta:o.theta,phi:o.phi,radius:o.radius},t:{x:tg.x,y:tg.y,z:tg.z},fov,max:mv.getMaximumFieldOfView()};}}catch(e){}}
      ...
      function propagateFrom(mv,orbit,target,fov){
        const srcInit=initialOrbit.get(mv)||orbit,srcInitT=initialTarget.get(mv)||{x:0,y:0,z:0},srcInitFov=initialFov.get(mv)||fov,srcInitMax=initialMaxFov.get(mv)||fov;
        const srcInitR=srcInit.radius||orbit.radius;
        const radiusRatio=orbit.radius/srcInitR;
        const fovRatio=fov/srcInitFov;  // current approach
        ...
        v.forEach(other=>{if(other===mv)return;const oI=initialOrbit.get(other),oIT=initialTarget.get(other),oIF=initialFov.get(other),oIMax=initialMaxFov.get(other);if(!oI||!oIT||!oIF)return;const oIR=oI.radius,oR=oIR*radiusRatio;
          const otherFov = oIF * fovRatio;
  ...
  console.log("  maxA="+iA.max.toFixed(4)+" maxB="+iB.max.toFixed(4));
  // Hypothesis: actual fov after zoom = max * (goalLogFov - log(max)) relationship
  console.log("  log(fovA/maxA)="+(Math.log(s.a.fov/iA.max)).toFixed(4)+" log(fovB/maxB)="+(Math.log(s.b.fov/iB.max)).toFixed(4));
  await browser.close();

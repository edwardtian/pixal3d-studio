...
(async()=>{
  ...
  await page.evaluate(()=>{
    window.setupCompareSyncInContainer = function(container){
      ...
      function propagateFrom(mv,orbit,target,fov){
        ...
        // Use the SAME coupling model-viewer uses: radius spans [minRadius, maxRadius] in sync with logFov spanning [log(minFov), log(maxFov)].
        // deltaZoom = log(fov) - log(srcInitFov)  (since srcInitFov == srcInitMax at start)
        const deltaZoom = Math.log(fov) - Math.log(srcInitFov);
        ...
        v.forEach(other=>{if(other===mv)return;const oI=initialOrbit.get(other),oIT=initialTarget.get(other),oIF=initialFov.get(other),oIMax=initialMaxFov.get(other),oIMin=initialMinFov.get(other);if(!oI||!oIT||!oIF)return;const oIR=oI.radius,oR=oIR*radiusRatio;
          const otherFov = oIF * Math.exp(deltaZoom);
  ...
  console.log("[ZOOM exp(deltaZoom) approach]");
  ...
  // compare log ratios
  console.log("  log(fovA/initA)="+(Math.log(s.a.fov/iA.fov)).toFixed(4)+" log(fovB/initB)="+(Math.log(s.b.fov/iB.fov)).toFixed(4));
  await browser.close();

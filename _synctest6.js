...
(async()=>{
  ...
  await page.evaluate(()=>{
    window.setupCompareSyncInContainer = function(container){
      ...
      const initialOrbit=new Map(),initialTarget=new Map(),initialFov=new Map(),initialMaxFov=new Map(),initialMinFov=new Map();
      const MIN_ZOOM_RADIUS_RATIO=0.2,MIN_ZOOM_FOV_RATIO=0.4;
      function recordInitial(mv){if(initialOrbit.has(mv)||!mv.loaded)return;try{const o=mv.getCameraOrbit(),tg=mv.getCameraTarget(),fov=mv.getFieldOfView();if(o&&o.radius>0){initialOrbit.set(mv,{theta:o.theta,phi:o.phi,radius:o.radius});initialTarget.set(mv,{x:tg.x,y:tg.y,z:tg.z});initialFov.set(mv,fov);initialMaxFov.set(mv,mv.getMaximumFieldOfView());initialMinFov.set(mv,mv.getMinimumFieldOfView());mv.minCameraOrbit='auto auto '+(o.radius*MIN_ZOOM_RADIUS_RATIO).toFixed(4)+'m';mv.minFieldOfView=''+(fov*MIN_ZOOM_FOV_RATIO).toFixed(2)+'deg';window.__init=window.__init||{};window.__init[mv.id]={o:{theta:o.theta,phi:o.phi,radius:o.radius},t:{x:tg.x,y:tg.y,z:tg.z},fov,max:mv.getMaximumFieldOfView(),min:mv.getMinimumFieldOfView()};}}catch(e){}}
      ...
      function propagateFrom(mv,orbit,target,fov){
        ...
        // log-space ratio: how far the source fov has moved from its max toward its min
        const logSpan = Math.log(srcInitFov) - Math.log(srcInitMin);
        const logFrac = logSpan > 1e-6 ? (Math.log(srcInitFov) - Math.log(fov)) / logSpan : 0;
        ...
        v.forEach(other=>{if(other===mv)return;const oI=initialOrbit.get(other),oIT=initialTarget.get(other),oIF=initialFov.get(other),oIMax=initialMaxFov.get(other),oIMin=initialMinFov.get(other);if(!oI||!oIT||!oIF)return;const oIR=oI.radius,oR=oIR*radiusRatio;
          ...
          const otherFov = otherLogSpan > 1e-6 ? oIF * Math.exp(-logFrac * otherLogSpan) : oIF * (fov/srcInitFov);
  ...
  await page.evaluate(()=>{const a=document.getElementById('a'),b=document.getElementById('b');a.cameraOrbit='0deg 75deg auto';b.cameraOrbit='0deg 75deg auto';a.fieldOfView='auto';b.fieldOfView='auto';a.jumpCameraToGoal();b.jumpCameraToGoal();});await wait(1500);
  ...
  // expected: B source zoomed in. other (A) should match the zoom fraction
  console.log("  A fov should be ~", (iA.fov * (s.b.fov/iB.fov)).toFixed(4), "if linear, or log-mapped");
  await browser.close();
